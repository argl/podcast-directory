defmodule Feeds.PodcastManager do
  ...
  def handle_call({:start_podcast, %Feeds.Podcast.Meta{_id: id}=_podcast_info}, _from, state) when is_nil(id) do
    {:reply, {:error, :invalid_podcast_info}, state}
  end
  def handle_call({:start_podcast, %Feeds.Podcast.Meta{_id: id}=podcast_info}, _from, state) do
